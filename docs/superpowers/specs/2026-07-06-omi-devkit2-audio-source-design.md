...
- **Pairing-time codec read deferred to first session connect.** The pair sheet (`OmiPairSheet`/`AppModel.pairOmi`) stores only the discovered device's identifier and name — it never reads the codec characteristic during pairing itself. The codec value is read (and validated) the first time a session actually connects to the paired device (`OmiAudioSource.handle(.connected(codecValue:))`); unsupported firmware surfaces through `omiSetupFailure`/`setupFailureMessage` at that point, not at pairing time.
- **Settings connection status/battery are session-gated by design.** There is no BLE traffic — and so nothing for `connectionStates()`/`batteryLevels()` to carry — until a session actually starts (`OmiAudioSource.start()` is what connects the transport). Settings' status/battery rows are therefore only ever live DURING an active session, not before one starts or after one ends. This is a deliberate deviation from the spec's implied always-live status, accepted to avoid keeping the BLE radio (and its battery cost) engaged while nothing is listening.
