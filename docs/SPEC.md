...
4. **Schedule the fallback local notification now** ("Sotto was paused. Tap to resume."), not on `.ended` — Apple documents that a `.began` is **not guaranteed** to get a matching `.ended`. Dedupe/cancel it if resume happens first. (Amendment 2026-07-23: notification authorization is requested as full [.alert, .sound], not provisional — the capture-unavailable alert must deliver loudly; see the failover-redesign spec.)
...
- **Layout:** title = time + duration; metadata row (backend, word count, speakers if diarized; M12 adds a Source chip — e.g. "Omi"/"iPhone mic" — shown only when the segment carries a non-default recorded source, so pre-M12 and phone-mic-only entries render exactly as before); full transcript — Deepgram version renders speaker turns as paragraphs with bold labels; audio player (scrubber, ±15 s, playback speed) shown only if audio retained. Playback must not disturb the live pipeline (playback through the same `.playAndRecord` session; pause Listening is _not_ required).
...
- **Listening:** audio source readout (auto-prefers a paired Omi wearable, with automatic failover to the iPhone mic when it's unreachable — no manual picker; shows "iPhone microphone" when nothing is paired, or "\<name\> + iPhone mic fallback" when it is); VAD threshold slider (0.1–0.9, labeled Sensitive ↔ Strict, "reset to default"); silence timeout; ring buffer; min segment. Power-user block collapsed by default.
- **Omi Device** (M12): pair/forget an Omi wearable. Unpaired: "Pair Omi Device…" button (scans by service UUID) + explainer copy. Paired: device name; connection status (Connecting…/Connected/Streaming/Not connected/Bluetooth off/permission needed) — live only DURING an active listening session (idle shows "Connects when listening" plus a caption, never a bare "Not connected" — redesign 2026-07-23), since there's no BLE traffic (and so nothing to report) before Start, a deliberate battery-motivated deviation from an always-live status; battery percentage when known; inline setup-failure text (e.g. unsupported firmware codec); "Forget This Device" (destructive, confirms). Pair/forget apply immediately if nothing is listening, otherwise once the current session ends — no relaunch required.
